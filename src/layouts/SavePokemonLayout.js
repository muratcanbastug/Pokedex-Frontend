...
const SavePokemonLayout = (props) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    let response = await savePokemon(newPokemonInfo, setErrorMessage, setError);
    if (response.status === 200) {
      let responseFileStatus = await postImage(selectedFile, pokemonName);
      if (responseFileStatus === 200) {
        const id = response.data.id;
        await sendNotification(id, pokemonName);
...
export default SavePokemonLayout;
